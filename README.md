...
# Lancer la stack (rebuild complet conseillé après pull)
...
## 11) Dépannage
- Erreur `RuntimeError: Form data requires "python-multipart"` : reconstruire l’image (`docker compose build --no-cache`) ou installer `python-multipart` dans l’environnement Python utilisé pour l’API.
